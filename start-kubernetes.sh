...
sudo docker run -it -d --privileged --name=kubernetes-master --hostname=kubernetes-master kiwenlau/kubernetes-cluster:1.0.7 supervisord --configuration=/etc/supervisor/conf.d/kubernetes-master.conf > /dev/null 
...
do
  ...
  sudo docker run -it -d --privileged --link kubernetes-master:kubernetes-master -e "ENV_KUBERNETES_MASTER=http://kubernetes-master:8080" -e "KUBERNETES_MASTER=http://kubernetes-master:8080" --privileged  --name=kubernetes-slave$i --hostname=kubernetes-slave$i kiwenlau/kubernetes-cluster:1.0.7 supervisord --configuration=/etc/supervisor/conf.d/kubernetes-slave.conf > /dev/null 
  ((i++))
done 
